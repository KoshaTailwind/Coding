# 👋 Привет, я Лиза!

Data Scientist с уникальным бэкграундом в психологии и менеджменте

## 🎯 О себе

Медицинский психолог по образованию (БГУ) с 5-летним опытом работы. Прошла путь от психолога до менеджера по продажам электронных компонентов, где открыла для себя мир данных. Сейчас осваиваю Data Science на платформе PRODUCTSTAR и подработаю 2D-иллюстратором.

## 🛠 Технический стек

`Python` `NumPy` `Pandas` `Jupyter Notebook` `VS Code` `Matplotlib` `Seaborn` `SQL` `PostgreSQL` `GitHub`

## 🎮 Мои проекты

### 🧪 Алхимический симулятор (Python RPG)

[![View Code](https://img.shields.io/badge/VIEW-CODE-blue?style=for-the-badge)](https://github.com/KoshaTailwind/Coding/blob/main/Coding/colored_potion.py)

### 🐶 [Анализ пород собак Stanford Dogs](https://github.com/KoshaTailwind/Coding/blob/main/Coding/DogBreed_PhotoAnalys.ipynb)
**Полный ETL пайплайн обработки 20,000+ изображений**

### 📊 Анализ данных
Примеры анализа и визуализации данных в Jupyter Notebook

**Игровое приложение для варки зелий с системой крафта**
```python
# Особенности проекта:
- Интерактивная текстовая RPG с меню
- Система рецептов с сохранением в файл
- Кастомизация через пользовательские рецепты
- Валидация ввода и обработка ошибок
